...
const {
  ...
  getMatchedRule
} = require('svg-sprite-loader/lib/utils') // move to local ?
const { NAMESPACE } = require('svg-sprite-loader/lib/config') // move to local ?
...
class SVGSpritePlugin {
  constructor (cfg = {}) {
    ...
    this.factory = ({ symbols }) => {
      ...
      console.log('opts', opts, spriteFactory(opts))
  ...
  additionalAssets (compilation) {
    ...
    console.log('itemsBySprite', itemsBySprite)
    // console.log('filenames', filenames)

    return Promise.map(filenames, (filename) => {
      const spriteSymbols = itemsBySprite[filename].map(item => item.symbol)

      console.log('spriteSymbols', spriteSymbols)
      ...
      return Sprite.create({
        ...
        .then((sprite) => {
          ...
          console.log('filename', `${this.filenamePrefix}${filename}`)
